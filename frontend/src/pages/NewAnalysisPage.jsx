...
const CATEGORY_COLORS = {
  conocimiento:  { bg: 'bg-blue-50',    text: 'text-blue-700',   border: 'border-blue-200',   dot: 'bg-blue-500' },
  recomendacion: { bg: 'bg-emerald-50', text: 'text-emerald-700',border: 'border-emerald-200',dot: 'bg-emerald-500' },
  comparativa:   { bg: 'bg-amber-50',   text: 'text-amber-700',  border: 'border-amber-200',  dot: 'bg-amber-500' },
  reputacion:    { bg: 'bg-red-50',     text: 'text-red-700',    border: 'border-red-200',    dot: 'bg-red-500' },
  servicios:     { bg: 'bg-purple-50',  text: 'text-purple-700', border: 'border-purple-200', dot: 'bg-purple-500' },
  precio:        { bg: 'bg-pink-50',    text: 'text-pink-700',   border: 'border-pink-200',   dot: 'bg-pink-500' },
}
  ...
  if (phase === 'suggesting') {
    return (
      <div className="max-w-lg mx-auto py-20 text-center">
        ...
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Generando preguntas personalizadas</h2>
        <p className="text-gray-400 text-sm">
          La IA está creando preguntas específicas para <strong className="text-gray-600">{business?.name}</strong>
          {' '}en el sector <strong className="text-gray-600">{form.sector}</strong>…
        </p>
          ...
          {PROGRESS_STEPS.map((step, i) => {
            ...
            return (
              <div key={step.key} className={`flex items-center gap-4 p-4 rounded-xl border transition-all ${
                done ? 'bg-emerald-50 border-emerald-100' : active ? 'bg-indigo-50 border-indigo-200' : 'bg-white border-gray-100'
              }`}>
    ...
    return (
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Revisá las preguntas sugeridas</h1>
        ...
        <div className="flex items-center justify-between bg-white border border-gray-100 rounded-xl px-5 py-3 mb-5 shadow-sm">
          <div className="flex items-center gap-4">
            <span className="text-sm text-gray-600">
              <span className="font-bold text-indigo-600 text-base">{totalSelected}</span>
              <span className="text-gray-400"> / {totalAvailable} seleccionadas</span>
            </span>
          ...
          {Object.entries(selection).map(([cat, questions]) => {
            ...
            return (
              <div key={cat} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <div className={`flex items-center justify-between px-5 py-3 border-b border-gray-100 ${colors.bg}`}>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">Mejora las preguntas comparativas.</p>
            </div>
            ...
            <div key={i} className="flex gap-2">
              ...
              {form.competitors.length > 1 && (
                <button type="button" onClick={() => removeCompetitor(i)}
                  className="text-gray-300 hover:text-red-400 transition-colors px-2 text-xl leading-none">×</button>
              )}
            </div>
          ))}
        </div>
      ...
      <div className="flex-1 min-w-0">
        {question.editing ? (
          <input
            ...
            className="w-full text-sm text-gray-800 border-b-2 border-indigo-400 outline-none bg-transparent pb-0.5"
            placeholder="Escribí la pregunta…"
          />
        ) : (
          <p
            onClick={onStartEdit}
            className="text-sm text-gray-700 cursor-text hover:text-gray-900 leading-relaxed"
            title="Clic para editar"
          >
            {question.prompt || <span className="italic text-gray-400">Pregunta vacía — clic para editar</span>}
          </p>
        )}
      </div>
